...
Processing inside the transaction verification and fraud detection services is partially asynchronous to allow for flexibility and parallel work; communication between the services themselves is synchronous. 
Inside the service, transaction verification does two asynchronous sub-calls, both with concurrency. 
Fraud detection, in the fully functional form, would utilise synchronised communication with an external service; it currently makes asynchronous sub-calls and checks them synchronously.
Suggestions would, in a real implementation, also require synchronous communication with an external service.
